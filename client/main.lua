...
AddEventHandler('QRCore:Client:OnPlayerLoaded', function()
    ...
    if PlayerJob and PlayerJob.name ~= "marshal" then
        if DutyBlips then
...
RegisterNetEvent('QRCore:Client:OnJobUpdate', function(JobInfo)
    if JobInfo.name == "marshal" and PlayerJob.name ~= "marshal" then
        CreatePrompts()
...
RegisterNetEvent('marshal:client:sendBillingMail', function(amount)
...
RegisterNetEvent('marshal:client:UpdateBlips', function(players)
    if Config.ShowBlips then
        if PlayerJob and (PlayerJob.name == 'marshal' or PlayerJob.name == 'ambulance') and onDuty then
            if DutyBlips then
                for k, v in pairs(DutyBlips) do
                    RemoveBlip(v)
                end
...
RegisterNetEvent('marshal:client:marshalAlert', function(coords, text)
    QRCore.Functions.Notify({text = text, caption = street1name.. ' ' ..street2name}, 'marshal')
    local transG = 250
...
RegisterNetEvent('marshal:client:SendToJail', function(time)
    TriggerServerEvent("marshal:server:SetHandcuffStatus", false)
    isHandcuffed = false
    isEscorted = false
    ClearPedTasks(PlayerPedId())
...
RegisterNetEvent('marshal:client:SendmarshalEmergencyAlert', function()
    local Player = QRCore.Functions.GetPlayerData()
    TriggerServerEvent('marshal:server:marshalAlert', Lang:t('info.officer_down', {lastname = Player.charinfo.lastname, callsign = Player.metadata.callsign}))

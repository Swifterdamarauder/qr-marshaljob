...
local function IsTargetDead(playerId)
    local retval = false
    QRCore.Functions.TriggerCallback('marshal:server:isPlayerDead', function(result)
        retval = result
    end, playerId)
    Wait(100)
    return retval
...
-- Events
RegisterNetEvent('marshal:client:SetOutVehicle', function()
...
RegisterNetEvent('marshal:client:PutInVehicle', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('marshal:client:SearchPlayer', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("inventory:server:OpenInventory", "otherplayer", playerId)
        TriggerServerEvent("marshal:server:SearchPlayer", playerId)
    else
        QRCore.Functions.Notify(Lang:t("error.none_nearby"), 'error')
    end
end)

RegisterNetEvent('marshal:client:SeizeCash', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("marshal:server:SeizeCash", playerId)
    else
        QRCore.Functions.Notify(Lang:t("error.none_nearby"), 'error')
    end
end)

RegisterNetEvent('marshal:client:SeizeDriverLicense', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("marshal:server:SeizeDriverLicense", playerId)
    else
        QRCore.Functions.Notify(Lang:t("error.none_nearby"), 'error')
    end
end)
...
RegisterNetEvent('marshal:client:RobPlayer', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    local ped = PlayerPedId()
    if player ~= -1 and distance < 2.5 then
...
RegisterNetEvent('marshal:client:JailCommand', function(playerId, time)
    TriggerServerEvent("marshal:server:JailPlayer", playerId, tonumber(time))
end)

RegisterNetEvent('marshal:client:BillCommand', function(playerId, price)
    TriggerServerEvent("marshal:server:BillPlayer", playerId, tonumber(price))
end)

RegisterNetEvent('marshal:client:JailPlayer', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        local dialogInput = LocalInput(Lang:t('info.jail_time_input'), 11)
        if tonumber(dialogInput) > 0 then
            TriggerServerEvent("marshal:server:JailPlayer", playerId, tonumber(dialogInput))
        else
...
RegisterNetEvent('marshal:client:BillPlayer', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        local dialogInput = LocalInput(Lang:t('info.jail_time_input'), 11)
        if tonumber(dialogInput) > 0 then
            TriggerServerEvent("marshal:server:BillPlayer", playerId, tonumber(dialogInput))
        else
...
RegisterNetEvent('marshal:client:PutPlayerInVehicle', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent("marshal:server:PutPlayerInVehicle", playerId)
        end
    else
        QRCore.Functions.Notify(Lang:t("error.none_nearby"), 'error')
    end
...
RegisterNetEvent('marshal:client:SetPlayerOutVehicle', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent("marshal:server:SetPlayerOutVehicle", playerId)
        end
    else
        QRCore.Functions.Notify(Lang:t("error.none_nearby"), 'error')
    end
...
RegisterNetEvent('marshal:client:EscortPlayer', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent("marshal:server:EscortPlayer", playerId)
        end
    else
        QRCore.Functions.Notify(Lang:t("error.none_nearby"), 'error')
    end
...
RegisterNetEvent('marshal:client:KidnapPlayer', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not IsPedInAnyVehicle(GetPlayerPed(player)) then
            if not isHandcuffed and not isEscorted then
                TriggerServerEvent("marshal:server:KidnapPlayer", playerId)
            end
        end
    else
        QRCore.Functions.Notify(Lang:t("error.none_nearby"), 'error')
...
RegisterNetEvent('marshal:client:CuffPlayerSoft', function()
    if not IsPedRagdoll(PlayerPedId()) then
        local player, distance = QRCore.Functions.GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            local playerId = GetPlayerServerId(player)
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                TriggerServerEvent("marshal:server:CuffPlayer", playerId, true)
                -- HandCuffAnimation()
            else
...
RegisterNetEvent('marshal:client:CuffPlayer', function()
    if not IsPedRagdoll(PlayerPedId()) then
        local player, distance = QRCore.Functions.GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            QRCore.Functions.TriggerCallback('QRCore:HasItem', function(result)
                if result then
                    local playerId = GetPlayerServerId(player)
                    if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                        TriggerServerEvent("marshal:server:CuffPlayer", playerId, false)
                        -- HandCuffAnimation()
                    else
                        QRCore.Functions.Notify(Lang:t("error.vehicle_cuff"), 'error')
                    end
...
RegisterNetEvent('marshal:client:GetEscorted', function(playerId)
    local ped = PlayerPedId()
...
RegisterNetEvent('marshal:client:DeEscort', function()
    isEscorted = false
    TriggerEvent('hospital:client:isEscorted', isEscorted)
    DetachEntity(PlayerPedId(), true, false)
end)

RegisterNetEvent('marshal:client:GetKidnappedTarget', function(playerId)
    local ped = PlayerPedId()
    QRCore.Functions.GetPlayerData(function(PlayerData)
...
RegisterNetEvent('marshal:client:GetKidnappedDragger', function(playerId)
    QRCore.Functions.GetPlayerData(function(PlayerData)
...
RegisterNetEvent('marshal:client:GetCuffed', function(playerId, isSoftcuff)
    local ped = PlayerPedId()
    if not isHandcuffed then
        isHandcuffed = true
        TriggerServerEvent("marshal:server:SetHandcuffStatus", true)
        ClearPedTasksImmediately(ped)
        ...
        TriggerEvent('hospital:client:isEscorted', isEscorted)
        DetachEntity(ped, true, false)
        TriggerServerEvent("marshal:server:SetHandcuffStatus", false)
        ClearPedTasksImmediately(ped)
